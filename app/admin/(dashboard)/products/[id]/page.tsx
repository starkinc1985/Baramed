    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            {specs.map((s, i) => (
              <div key={i} className="flex gap-2">
                <input placeholder="Key" value={s.key} onChange={(e) => { const n = [...specs]; n[i].key = e.target.value; setSpecs(n); }} className={INPUT} />
        ...
        <div className="rounded-xl border border-stroke bg-white p-6 dark:border-strokedark dark:bg-blacksection">
          ...
          {parents.length === 0 ? <p className="text-sm text-waterloo">No categories available.</p> : (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {parents.map((parent) => {
