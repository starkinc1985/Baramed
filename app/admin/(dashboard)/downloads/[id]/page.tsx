...
export default function EditDownloadPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", category: "OTHER", fileSize: "", fileType: "", downloadUrl: "", published: true, sortOrder: "0" });
  const [status, setStatus] = useState<"idle" | "saving" | "deleting" | "error">("idle");
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch(`/api/admin/downloads/${id}`).then((r) => r.json()).then((d) => {
      const dl = d.download;
      setForm({ title: dl.title, description: dl.description ?? "", category: dl.category, fileSize: dl.fileSize ?? "", fileType: dl.fileType ?? "", downloadUrl: dl.downloadUrl, published: dl.published, sortOrder: String(dl.sortOrder) });
      setLoading(false);
    });
  }, [id]);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="h-4 w-24 animate-pulse rounded bg-gray-200 dark:bg-strokedark" />
          <div className="h-7 w-36 animate-pulse rounded bg-gray-200 dark:bg-strokedark" />
        </div>
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-black dark:text-white">Edit Download</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg space-y-4 rounded-xl border border-stroke bg-white p-6 dark:border-strokedark dark:bg-blacksection">
        ...
        <div><label className={LABEL}>Download URL *</label><input required value={form.downloadUrl} onChange={(e) => setForm({ ...form, downloadUrl: e.target.value })} className={INPUT} /></div>
        <div className="grid gap-4 md:grid-cols-2">
          <div><label className={LABEL}>File Type</label><input value={form.fileType} onChange={(e) => setForm({ ...form, fileType: e.target.value })} className={INPUT} /></div>
          <div><label className={LABEL}>File Size</label><input value={form.fileSize} onChange={(e) => setForm({ ...form, fileSize: e.target.value })} className={INPUT} /></div>
        </div>
